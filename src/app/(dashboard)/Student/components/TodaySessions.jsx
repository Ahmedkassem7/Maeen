...
import { Badge, badgeVariants } from "@/app/_component/ui/Badge";
import {
  Card,
  CardContent,
  CardDescription,
...
import { BookOpen, Calendar, Video, AlertCircle } from "lucide-react";
import ErrorDisplay from "@/app/_component/shared/ErrorDisplay";
import useAuthStore from "@/stores/AuthStore";
import { useEffect } from "react";
import { getHalakatStudentToday, getHalakatToday } from "@/app/Api/session";
...
export default function TodaySessions({ todaySessions = [] }) {
  ...
  const { studentTodayHalakat, fetchHalakatStudentToday, error, loading } =
    useHalakaSessionStore();
  useEffect(() => {
    fetchHalakatStudentToday(token);
  }, [token]);
  ...
  if (!studentTodayHalakat || studentTodayHalakat.length === 0) {
    return (
      <Card className="mb-6 lg:mb-8 border-0 shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-islamic-blue to-blue-600 p-4 lg:p-6">
  ...
  return (
    <Card className="mb-6 lg:mb-8 border-0 shadow-lg overflow-hidden">
      <div className="bg-gradient-to-r from-islamic-blue to-blue-600 p-4 lg:p-6">
        <CardHeader className="p-0">
          <CardDescription className="text-blue-100 mt-1 lg:mt-2 text-sm lg:text-base flex items-center">
            <Calendar className="h-5 w-5 lg:h-6 lg:w-6 ml-2 lg:ml-3" />
      ...
      <CardContent className="p-4 lg:p-6">
        <div className="space-y-3 lg:space-y-4">
          {studentTodayHalakat.map((session) => (
            <div
              key={session.id}
              className="group relative bg-gradient-to-r from-white to-gray-50 rounded-xl lg:rounded-2xl border border-gray-100 hover:shadow-lg transition-all duration-300 hover:border-islamic-blue/20 overflow-hidden"
            >
              {/* Mobile Layout (< sm) */}
              <div className="block sm:hidden p-4">
                <div className="flex items-start gap-3 mb-4">
                  <div className="relative flex-shrink-0">
                    <div className="w-12 h-12 bg-gradient-to-br from-islamic-blue to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
                      <BookOpen className="h-6 w-6 text-white" />
                    </div>
                    <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-islamic-gold rounded-full flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-islamic-blue text-base mb-1 truncate">
                      {session.title}
                    </h3>
              ...
              <div className="hidden sm:block p-4 lg:p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 lg:gap-4 flex-1 min-w-0">
                    <div className="relative flex-shrink-0">
                      <div className="w-12 h-12 lg:w-16 lg:h-16 bg-gradient-to-br from-islamic-blue to-blue-600 rounded-xl lg:rounded-2xl flex items-center justify-center shadow-lg">
                        <BookOpen className="h-6 w-6 lg:h-8 lg:w-8 text-white" />
                      </div>
                      <div className="absolute -bottom-1 -right-1 w-5 h-5 lg:w-6 lg:h-6 bg-islamic-gold rounded-full flex items-center justify-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-islamic-blue text-lg lg:text-xl mb-2 truncate">
                        {session.title}
                      </h3>
                      <div className="flex flex-wrap items-center gap-2 lg:gap-4 text-sm text-gray-600">
                        ...
                        <Badge
                          variant="outline"
                          className="border-islamic-gold text-islamic-gold bg-islamic-gold/10"
                        >
