import { Search, User, MessageCircle, Clock, RefreshCw } from "lucide-react";
  ...
  ({ conversation, isSelected, isOnline, unreadCount, onClick }) => {
    ...
    return (
      <div
        ...
        ${
          isSelected
            ? "bg-gradient-to-r from-[#0b1b49] to-[#0b1b49]/90 text-white shadow-lg"
      ...
      >
        ...
        <div className="flex items-center space-x-3 rtl:space-x-reverse">
          <Avatar
            ...
            isOnline={isOnline}
            showRole={true}
            className={isSelected ? " ring-white ring-opacity-30" : ""}
          />
        ...
        {!isSelected && (
          <div className="absolute inset-0 bg-gradient-to-r from-[#0b1b49]/5 to-transparent rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        )}
      </div>
    );
  }
...
const ConversationsList = ({
  ...
  onRefresh, // Add refresh prop
}) => {
  // فقط المحادثات التي بها اسم حقيقي
  const realConversations = filteredConversations.filter(
    (conv) =>
      conv.user &&
      conv.user.firstName &&
      conv.user.firstName !== "اسم غير معروف"
  );
  const totalUnreadCount = realConversations.reduce(
  ...
  return (
    <div className="w-full md:w-96 bg-white border-l border-gray-200 flex flex-col flex-shrink-0 md:h-full shadow-sm">
      {/* Header */}
      <div className="p-4 bg-gradient-to-r from-[#0b1b49] to-[#0b1b49]/90 text-white">
            ...
            <>
              ...
              <div className="mt-4 p-3 bg-white rounded-lg border border-gray-200">
                <div className="flex items-center justify-between text-sm text-gray-600">
                  ...
                  <span className="font-semibold text-[#0b1b49]">
                  ...
                  <span className="font-semibold text-green-600">
                    {
                      realConversations.filter(
                        (conv) =>
                          conv.user && onlineUsers.includes(conv.user._id)
                      ).length
                    }
                  </span>
                </div>
