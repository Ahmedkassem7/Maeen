...
const AddHalaqaModal = ({ studentId, studentName }) => {
  ...
  const initialValues = {
    title: "",
    description: "",
    halqaType: studentId ? "private" : "halqa", // إذا كان هناك studentId، اجعلها خاصة
    maxStudents: 10,
    price: "",
    student: studentId || "687b9fec8d65dbc8d5f10804", // استخدم studentId الممرر
  ...
  const validationSchema = Yup.object({
    ...
    student: Yup.string().when("halqaType", {
      is: "private",
      then: (schema) => schema.required("يجب اختيار الطالب للحلقات الخاصة"),
      otherwise: (schema) => schema.notRequired(),
    }),
  ...
  const handleSubmit = async (values, { resetForm, setSubmitting }) => {
    try {
      ...
      setToastState({
        show: true,
        message: "فشل في إنشاء الحلقة. يرجى المحاولة مرة أخرى.",
        type: "error",
        duration: 2000,
      });
    } finally {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-gradient-to-r from-islamic-blue to-blue-600 text-white hover:shadow-lg transition-all duration-300">
          ...
          {studentId ? "إضافة حلقة خاصة" : "إضافة حلقة جديدة"}
        </Button>
      </DialogTrigger>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <DialogTitle className="text-3xl font-bold mb-2">
                {studentId ? "إنشاء حلقة خاصة" : "إنشاء حلقة جديدة"}
              </DialogTitle>
              <DialogDescription className="text-blue-100 text-lg">
                {studentId
                  ? `قم بإعداد حلقة خاصة مع ${studentName || "الطالب"}`
                  : "قم بإعداد حلقة قرآنية جديدة بخطوات بسيطة ومنظمة"}
              </DialogDescription>
            </div>
          </div>
        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          {studentId && (
                            <p className="text-sm text-blue-600 bg-blue-50 p-2 rounded-lg">
                              💡 تم تحديد نوع الحلقة كـ "خاصة" تلقائياً لأنك في
                              محادثة مع طالب
                            </p>
                          )}
                        </div>
                    ...
                    {values.halqaType === "private" ? (
                      ...
                      <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
                        ...
                        <p className="text-blue-700 text-sm">
                          السعر سيتم تحديده تلقائياً من إعدادات ملفك الشخصي. هذه
                          الحلقة مخصصة لطالب واحد فقط.
                        </p>
                        {studentName && (
